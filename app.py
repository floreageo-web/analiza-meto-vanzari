...
with t1:
    ...
    with col1: st.plotly_chart(px.line(df_view, x="Data", y="Max", color="Oras", title="Temp Maxime (°C)"), use_container_width=True)
    with col2: st.plotly_chart(px.bar(df_view, x="Data", y="Precipitatii", color="Oras", title="Precipitatii (mm)"), use_container_width=True)

with t2:
    st.subheader("⚖️ Analiza Comparativa")
    if not sel_oras: st.warning("Selecteaza un oras."); st.stop()
    c1, c2 = st.columns(2)
    with c1: int_a = st.date_input("Perioada A", [df["Data"].min().date(), (df["Data"].min() + timedelta(days=6)).date()], key="ka")
    with c2: int_b = st.date_input("Perioada B", [df["Data"].max().date() - timedelta(days=6), df["Data"].max().date()], key="kb")
    ...
    if len(int_a) == 2 and len(int_b) == 2:
        def get_summary(start, end, city):
            ...
            mode_wmo = sub["WMO"].mode()[0] if not sub["WMO"].mode().empty else 0
            return {"max": sub["Max"].mean(), "prec": sub["Precipitatii"].sum(), "soare": sub["OreSoare"].mean(), "emoji": wmo_to_emoji(mode_wmo)[0]}
        
        sum_a = get_summary(int_a[0], int_a[1], sel_oras[0])
        sum_b = get_summary(int_b[0], int_b[1], sel_oras[0])
        
        if sum_a and sum_b:
            st.markdown(f"### Rezultate Comparatie: {sel_oras[0]}")
...
with t3:
    st.info("Harta locatiilor McDonald's.")
